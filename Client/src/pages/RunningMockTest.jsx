...
const RunningMockTest = () => {
  ...
  const completedCount = Object.values(techniqueChecks).filter(
    (v) => v !== "",
  ).length;
  const correctCount = Object.values(techniqueChecks).filter(
    (v) => v === "correct",
  ).length;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
                ...
                {runningChecksList.map((check) => (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        onClick={() =>
                          handleTechniqueChange(check.id, "correct")
                        }
                      ...
                      <button
                        onClick={() =>
                          handleTechniqueChange(check.id, "incorrect")
                        }
                        className={`py-3 px-4 rounded-xl border transition-all flex items-center gap-2 ${
          ...
          <div className="space-y-6">
            <div
              className={`rounded-[2.5rem] border ${getScoreBg(techniqueScore)} p-8 shadow-xl text-center`}
            >
              ...
              <div
                className={`text-7xl font-black ${getScoreColor(techniqueScore)} mb-4`}
              >
                {techniqueScore}
              </div>
            ...
            {techniqueChecks.faceExpression === "incorrect" && (
              <div className="bg-red-50 border-2 border-red-100 p-6 rounded-[2rem] animate-pulse">
                <div className="flex gap-4">
                  ...
                  />
                  <div>
                    <p className="font-black text-red-600 uppercase text-xs mb-1">
                      Injury Prevention Alert
                    </p>
                    <p className="text-sm text-red-800 font-medium">
                      Athlete is showing signs of pain or grimacing. Recommend
                      immediate rest and recovery check.
                    </p>
                  </div>
                </div>
              </div>
